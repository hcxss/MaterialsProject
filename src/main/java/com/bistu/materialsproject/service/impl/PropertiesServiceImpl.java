...
import com.bistu.materialsproject.service.PropertiesService;
...
public class PropertiesServiceImpl implements PropertiesService {
    ...
    @Override
    public Propertie findByMp_id(Integer mp_id) {
        Query query = new Query(Criteria.where("mp_id").is(mp_id)).limit(1);
        return mongoTemplate.findOne(query, Propertie.class);

    }

    @Override
    public List<Propertie> getProperties(int pageNum, int pageSize) {
        ...
        if (pageNum != 1) {
            ...
            List<Propertie> Properties = mongoTemplate.find(query, Propertie.class);
            // 取出最后一条
            Propertie Propertie = Properties.get(Properties.size() - 1);

            // 取到上一页的最后一条数据 id，当作条件查接下来的数据
            int id = Propertie.getMp_id();
        ...
        // 即可得到第n页数据
        List<Propertie> PropertieList = mongoTemplate.find(query, Propertie.class);
